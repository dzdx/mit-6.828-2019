...
bd_malloc(uint64 nbytes)
{
  ...
  bit_xor_set(bd_sizes[k].alloc, blk_index(k, p)/2);
  for(; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k-1);   // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_xor_set(bd_sizes[k-1].alloc, blk_index(k-1, p)/2);
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_xor_set(bd_sizes[k].alloc, bi/2);
...
int
bd_initfree_pair(int k, int bi, int end) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(bit_isset(bd_sizes[k].alloc, buddy/2)) {
    // one of the pair is free
    free = BLK_SIZE(k);
    int target = end? buddy: bi;
